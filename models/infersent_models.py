    ...
    def encode(self, sentences, bsize=64, maxlen=40, tokenize=True, verbose=False):
        ...
        for stidx in tqdm(range(0, len(sentences), bsize), disable=True):
            ...
            if self.is_cuda():
                batch = batch.cuda()
            batch = (
                self.forward((batch, lengths[stidx : stidx + bsize])).data.cpu().numpy()
            )
...
class BGRUlastEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.init_lstm = Variable(
            torch.FloatTensor(2, self.bsize, self.enc_lstm_dim).zero_()
        ).cuda()
    ...
    def forward(self, sent_tuple):
        ...
        self.init_lstm = (
            ...
            else Variable(torch.FloatTensor(2, bsize, self.enc_lstm_dim).zero_()).cuda()
        )
...
class BLSTMprojEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.proj_enc = nn.Linear(
            2 * self.enc_lstm_dim, 2 * self.enc_lstm_dim, bias=False
        )
    ...
    def forward(self, sent_tuple):
        ...
        # Un-sort by length
        idx_unsort = np.argsort(idx_sort)
        sent_output = sent_output.index_select(
            1, Variable(torch.cuda.LongTensor(idx_unsort))
        )
...
class LSTMEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.init_lstm = Variable(
            torch.FloatTensor(1, self.bsize, self.enc_lstm_dim).zero_()
        ).cuda()
    ...
    def forward(self, sent_tuple):
        ...
        self.init_lstm = (
            self.init_lstm
            if bsize == self.init_lstm.size(1)
            else Variable(torch.FloatTensor(1, bsize, self.enc_lstm_dim).zero_()).cuda()
        )
        ...
        # Handling padding in Recurrent Networks
        sent_packed = nn.utils.rnn.pack_padded_sequence(sent, sent_len)
        sent_output = self.enc_lstm(sent_packed, (self.init_lstm, self.init_lstm))[1][
            0
        ].squeeze(
            0
        )  # batch x 2*nhid
...
class InnerAttentionNAACLEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.enc_lstm = nn.LSTM(
            self.word_emb_dim, self.enc_lstm_dim, 1, bidirectional=True
        )
        self.init_lstm = Variable(
            torch.FloatTensor(2, self.bsize, self.enc_lstm_dim).zero_()
        ).cuda()
    ...
    def forward(self, sent_tuple):
        ...
        sent_output_proj = self.proj_lstm(
            sent_output.view(-1, 2 * self.enc_lstm_dim)
        ).view(bsize, -1, 2 * self.enc_lstm_dim)

        sent_key_proj = self.proj_key(sent_output.view(-1, 2 * self.enc_lstm_dim)).view(
        ...
        sent_w = self.query_embedding(
            Variable(torch.LongTensor(bsize * [0]).cuda())
        ).unsqueeze(
            2
        )  # (bsize, 2*nhid, 1)
...
class InnerAttentionMILAEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        sent_w2 = self.query_embedding(
            Variable(torch.LongTensor(bsize * [1]).cuda())
        ).unsqueeze(
            2
        )  # (bsize, nhid, 1)
...
class InnerAttentionYANGEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.proj_lstm = nn.Linear(
            2 * self.enc_lstm_dim, 2 * self.enc_lstm_dim, bias=True
        )
    ...
    def forward(self, sent_tuple):
        ...
        sent_keys = self.proj_enc(sent_output.view(-1, 2 * self.enc_lstm_dim)).view(
            bsize, -1, 2 * self.enc_lstm_dim
        )
...
class NLINet(nn.Module):
    def __init__(self, config):
        ...
        self.inputdim = (
            self.inputdim / 2 if self.encoder_type == "LSTMEncoder" else self.inputdim
        )
...
class ClassificationNet(nn.Module):
    def __init__(self, config):
        ...
        self.inputdim = (
            4 * self.inputdim
            if self.encoder_type == "ConvNetEncoder"
            else self.inputdim
        )
        self.inputdim = (
