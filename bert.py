...
def run_bigram_coherence(args):
    ...
    # print_current_time()
    if PRETRAINED_MODEL:
        model = torch.load(f"data/{name_key}_model.pt")
        ...
        torch.save(model, f"data/{name_key}_model.pt")
        model.load_best_state()
    ...
    # Save results
    results_path = os.path.join(RESULTS_SAVE_PATH, f"{name_key}-%.4f" % (valid_acc))
    results = {
